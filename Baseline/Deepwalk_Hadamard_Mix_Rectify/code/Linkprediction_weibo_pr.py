...
RunNumber = 1
Number = 20000
user_data = pd.read_csv('../data/origin_data/weibo/userList.txt', header=None)
...
def spilt_train_test():
    follow_data = pd.read_csv('../data/origin_data/weibo/followgraph.txt', header=None)
    G_origin = nx.DiGraph()
    ...
    _, test_noexist = train_test_split(no_exist_edge, test_size=0.0007)
    _, train_noexist = train_test_split(no_exist_edge, test_size=0.0065)
    G = nx.DiGraph()
...
def precise_and_recall(model, g, test_exist, node_vec, tim):
    ...
    fout = open(r'../result/weibo/precise_%s.txt'%tim, 'w', encoding='utf-8')
    for _ in zip(length_list, precise_):
        fout.write(str(_[0]) + ' ' + str(_[1]) + '\n')
    fout.close()
    fout = open(r'../result/weibo/recall_%s.txt'%tim, 'w', encoding='utf-8')
